...
User.init(// initialize the User model by calling the init() method on the User class
	{
		...
		username: {// define the username column with the following properties
			type: DataTypes.STRING,
			allowNull: false,
		},
		email: {// define the email column with the following properties
			type: DataTypes.STRING,
			allowNull: false,
			unique: true,
			validate: {
		...
		password: {// define the password column with the following properties
			type: DataTypes.STRING,
			allowNull: false,
			validate: {
				len: [6],
	...
	},
	{
		hooks: {// define the hooks object to run the beforeCreate() method before a new user is created
			async beforeCreate(newUserData) {// beforeCreate() is a sequelize hook that is used to hash the password before it is created
				newUserData.password = await bcrypt.hash(newUserData.password, 3);// hash() is used to hash the password, the first argument is the password to be hashed, the second argument is the number of times the password is hashed
